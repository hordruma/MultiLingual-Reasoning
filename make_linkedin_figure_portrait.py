...
plt.rcParams.update({"font.family": "DejaVu Sans", "font.size": 12.5, "axes.edgecolor": GRID,
                     "axes.labelcolor": INK2, "xtick.color": INK2, "ytick.color": INK2,
                     "text.color": INK, "figure.facecolor": SURF, "axes.facecolor": SURF})

fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 13), dpi=150, gridspec_kw={"height_ratios": [1, 1.15], "hspace": 0.5})
fig.suptitle("Does the language of the chain of thought change the answer?\nMostly no. But some languages stop the model from finishing.",
             x=0.04, ha="left", fontsize=16, fontweight="bold", y=0.985, linespacing=1.3)
fig.text(0.04, 0.912, "9 LegalBench classification tasks, 1,048 items per cell, paired against English on identical items,\ntemperature 0, no output cap.", fontsize=11, color=INK2, va="top")
...
ax1.text(-10.5, -1.0, "Each dot is one language or notation. Nothing beats English on any model;\nonly the oldest model (gpt-4.1-mini) loses more than 3 points.", fontsize=10.5, color=INK2, va="bottom")
...
ax2.legend(handles=[Line2D([], [], marker="s", ls="", color=c, markersize=10, label=nme) for nme, _, c in MODELS],
           loc="upper left", frameon=False, fontsize=10.5, ncol=1, handletextpad=0.4)
fig.text(0.04, 0.072, "* Luna's hidden reasoning is on; its visible text is a short write-up in the requested language and never loops.\n"
...
fig.text(0.04, 0.02, "github.com/hordruma/MultiLingual-Reasoning", fontsize=10, color=INK2)
fig.subplots_adjust(left=0.27, right=0.97, top=0.825, bottom=0.125)
