...
MODELS = {
    ...
    # Each of these is a fixed pull already on this box (see `ollama list`).
    # Unlike the generic "ollama" entry above, each has its own config key,
    # so results from different local models don't collide in results/ and
    # `--models ollama_qwen35_9b,ollama_gemma4_31b,...` runs them all in one
    # invocation (sequentially — see run_experiment.py's per-model loop).
    "ollama_qwen35_9b": {
        "provider": "openai_compat",
        "model_id": "qwen3.5:9b",
    ...
    # Gemma 4 has no documented way to disable its thinking on Ollama's /v1
    # endpoint, and returns it in the `reasoning` field with `content` left
    # empty; providers.split_reasoning() promotes that text into the answer.
    "ollama_gemma4_31b": {
        "provider": "openai_compat",
        "model_id": "gemma4:31b",
        "display": "Ollama local: gemma4:31b",
        "origin_country": "local",
        "api_key_env": "OLLAMA_API_KEY",
        "api_key_default": "ollama",
        "base_url_env": "OLLAMA_BASE_URL",
    ...
    # Not documented elsewhere in this repo; run --smoke-test before trusting
    # their reasoning/thinking behavior on the /v1 endpoint.
    "ollama_nemotron3nano_30b": {
        "provider": "openai_compat",
        "model_id": "nemotron-3-nano:30b",
        "display": "Ollama local: nemotron-3-nano:30b",
        "origin_country": "local",
        "api_key_env": "OLLAMA_API_KEY",
        "api_key_default": "ollama",
        "base_url_env": "OLLAMA_BASE_URL",
    ...
    "ollama_llama32_3b": {
        ...
        "display": "Ollama local: llama3.2:3b",
        "origin_country": "local",
        "api_key_env": "OLLAMA_API_KEY",
        "api_key_default": "ollama",
        "base_url_env": "OLLAMA_BASE_URL",
